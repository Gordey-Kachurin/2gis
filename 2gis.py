...
Для работы программы необходимо примерно 1 ГБ свободной оперативной памяти.
Во время работы браузер Firefox может несколько раз открываться и закрываться.
Закрытие браузера позволяет освободить память компьютера.
''' , end='') 
